...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    private void RemoveBuilding(Vector3Int gridPosition)
    {
        ...
        if (cell.IsOccupied)                                    //해당 위치에 건물이 있는지 확인한다.
        {
            ...
            cell.Building = null;               //Cell 에 놓인 빌딩 null 값으로 설정
        }
    }
    ...
    private Vector3 GetLookPosition()
    {
        ...
        {
            Vector3 chracterPosition = playerController.transform.position;         //플레이어 위치를 가져오고
            Vector3 chracterFoward = playerController.transform.forward;            //플레이어의 앞 방향
            Vector3 rayOrigin = chracterPosition + Vector3.up * 1.5f + chracterFoward * 0.5f;
            Vector3 rayDirection = (chracterFoward - Vector3.up).normalized;        //캐릭터가 보는 방향 앞 대각선
